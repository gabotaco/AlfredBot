	...
	return new Promise((resolve, reject) => {
		...
		function appendData(auth) {
			...
			if (car.toLowerCase() == 'random') {
				return resolve(
					`{"mods":{"1":${RandomNumber(-1, 2)},"2":${RandomNumber(
						-1,
						2
					)},"3":${RandomNumber(-1, 2)},"4":${RandomNumber(
						-1,
						2
					)},"5":${RandomNumber(-1, 1)},"6":${RandomNumber(
						-1,
						1
					)},"7":${RandomNumber(-1, 1)},"8":${RandomNumber(
						-1,
						1
				...
				sheets.spreadsheets.values.update(
					{
						//Puts what they typed into the search bar
						auth: auth,
						...
						if (err) {
							...
							sheets.spreadsheets.values.get(
								...
								(err, res) => {
									...
									if (!rows) {
										//if there aren't any values for the data
