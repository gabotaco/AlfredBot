...
const requestSerialize = value => {
	if (!(value instanceof AssistRequest.ctor)) {
		throw new Error(
			'Expected argument of type google.assistant.embedded.v1alpha2.AssistRequest'
		);
	}
...
const responseSerialize = value => {
	if (!(value instanceof AssistResponse.ctor)) {
		throw new Error(
			'Expected argument of type google.assistant.embedded.v1alpha2.AssistResponse'
		);
	}
...
// create the client
exports.EmbeddedAssistantClient = grpc.makeGenericClientConstructor(
	EmbeddedAssistantService
);
