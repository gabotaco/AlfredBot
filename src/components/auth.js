...
function Auth(config) {
	...
	if (config.keyFilePath === undefined) {
		throw new Error(
			'Missing "keyFilePath" from config (should be where your JSON file is)'
		);
	}

	if (config.savedTokensPath === undefined) {
		throw new Error(
			'Missing "savedTokensPath" from config (this is where your OAuth2 access tokens will be saved)'
		);
		return;
	}
	...
	const getTokens = () => {
		...
		open(url).catch(() => {
			console.log(
				'Failed to automatically open the URL. Copy/paste this in your browser:\n',
				url
			);
		});
