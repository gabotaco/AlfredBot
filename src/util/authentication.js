...
class Authentication {
  authenticate() { //Runs to get auth to use api
  ...
  authorize(credentials) {
    ...
    return new Promise((resolve, reject) => {
      ...
      fs.readFile(TOKEN_PATH, (err, token) => {
        if (err) { //If no file
          this.getNewToken(oauth2Client).then((oauth2ClientNew) => { //Make file with token
  ...
  getNewToken(oauth2Client, callback) {
    return new Promise((resolve, reject) => {
      var authUrl = oauth2Client.generateAuthUrl({ //make url to make a new token
        access_type: 'offline',
        scope: SCOPES
      });
      console.log('Authorize this app by visiting this url: \n ', authUrl);
      var rl = readline.createInterface({ //allows you to type in console
        input: process.stdin,
