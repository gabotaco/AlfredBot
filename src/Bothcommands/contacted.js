...
module.exports.run = async (bot, args) => {
  return new Promise((resolve, reject) => {
    if (args.id) {
      ...
      //If nobody specified
      bot.con.query(
        `SELECT * FROM applications WHERE status = 'Contacted'`,
        function (err, result, fields) {
          ...
          result.forEach((applicant) => {
            reqEmbed.addField(
              applicant.in_game_name + " (" + applicant.in_game_id + ")",
              applicant.status_info
            ); //Adds them to embed
          });
          if (!reqEmbed.fields[0]) {
            //if there aren't any fields in the embed
