...
module.exports.run = async (bot, args) => {
	return new Promise(async (resolve, reject) => {
		if (args.amount) {
			if (pendingPayouts[args.author_id])
				return resolve('Please confirm or deny your last payout.');
			...
			if (args.guild_id == botconfig.PIGSServer) {
				...
				var NewDeadline = function (MemberDetails) {
					...
					if (D3 >= -45 * 24 * 60 * 60 * 1000) {
						//45 days till deadline
						if (voucherAmount > 60000) {
							//turnin in a lot
							CurrentDeadline.setDate(
								CurrentDeadline.getDate() + Math.ceil(voucherAmount / 8000) + 19
							); //add 9
							return CurrentDeadline;
						} else if (voucherAmount <= 60000 && voucherAmount > 30000) {
							CurrentDeadline.setDate(
								CurrentDeadline.getDate() + Math.ceil(voucherAmount / 4000) + 15
							); //add 3 days
							return CurrentDeadline;
						} else if (voucherAmount <= 30000 && voucherAmount >= 15000) {
							CurrentDeadline.setDate(
								CurrentDeadline.getDate() + Math.ceil(voucherAmount / 2500) + 10
							); //add 3 days
							return CurrentDeadline;
						} else if (voucherAmount <= 15000 && voucherAmount >= 6000) {
							CurrentDeadline.setDate(
								CurrentDeadline.getDate() + Math.ceil(voucherAmount / 750) + 14
							); //add 3 days
							return CurrentDeadline;
						} else {
							CurrentDeadline.setDate(
								CurrentDeadline.getDate() + Math.ceil(voucherAmount / 250)
							);
						...
						if (voucherAmount > 60000) {
							...
							CurrentDeadline.setDate(
								CurrentDeadline.getDate() + Math.ceil(voucherAmount / 750) + 4
							); //add 3 days
							return CurrentDeadline;
						} else {
							CurrentDeadline.setDate(
								CurrentDeadline.getDate() + Math.ceil(voucherAmount / 2000)
							);
			...
			resolve([
				...
				args.slash
					? '<@' +
					  args.author_id +
					  '> Please `/payout (c)onfirm` or `/payout (d)eny` to perform this payout.'
					: 'Please `.payout (c)onfirm` or `.payout (d)eny` to perform this payout.',
			]);
			pendingPayouts[args.author_id] = {
				...
				NewDeadline: NewDeadline(MemberDetails)
					.toISOString()
					.slice(0, 19)
					.replace('T', ' '),
			...
			if (!pendingPayouts[args.author_id])
				return resolve('You have no pending payouts.');
			if (args.sub_command == 'deny') {
				//if cancelled
				...
				bot.con.query(
					`SELECT id FROM members WHERE discord_id = '${args.author_id}'`,
					function (err, member, fields) {
						...
						if (member.length == 0) {
							...
							bot.con.query(
								`UPDATE managers SET ${pendingPayout.CompanyName}_cashout = ${pendingPayout.CompanyName}_cashout + ${pendingPayout.voucherAmount}, ${pendingPayout.CompanyName}_cashout_worth = ${pendingPayout.CompanyName}_cashout_worth + ${pendingPayout.Money} WHERE member_id = '${member[0].id}'`,
								function (err, result, fields) {
									//update the manager with the new cashout
									if (err) {
										console.log(err);
										return reject(
											'Unable to updated the managers table. No data was added.'
										);
									}
									if (result.affectedRows == 0) {
										//if no manager
										bot.con.query(
											`INSERT INTO managers (member_id, ${pendingPayout.CompanyName}_cashout, ${pendingPayout.CompanyName}_cashout_worth) VALUES ('${member[0].id}', '${pendingPayout.voucherAmount}', '${pendingPayout.Money}')`,
											function (err) {
												if (err) {
													console.log(err);
									...
									bot.con.query(
										`UPDATE ${pendingPayout.CompanyName} SET vouchers = vouchers + '${pendingPayout.voucherAmount}', worth = worth + '${pendingPayout.Money}' WHERE member_id = '${pendingPayout.member_id}'`,
										function (err, result, fields) {
											if (err) {
												console.log(err);
												return reject(
													'Unable to add vouchers to the member. WARNING MANAGERS TABLE WAS UPDATED'
												);
											}
											//add to payout table
											bot.con.query(
												`INSERT INTO payout(manager_id, member_id, company, amount, worth) VALUES ('${member[0].id}', '${pendingPayout.member_id}', '${pendingPayout.CompanyName}', '${pendingPayout.voucherAmount}', '${pendingPayout.Money}')`,
												function (err) {
													if (err) {
														console.log(err);
														return reject(
															'Unable to add this payout to payouts table. WARNING MANAGERS AND COMPANY TABLE WERE UPDATED'
														);
													}
													//update deadline
													bot.con.query(
														`UPDATE members SET deadline = '${pendingPayout.NewDeadline}', last_turnin = '${pendingPayout.CurrentDate}' WHERE id = '${pendingPayout.member_id}'`,
														function (err, result, fields) {
															if (err) {
																console.log(err);
																return reject(
																	'Unable to update members deadline and last turning. WARNING MANAGERS, COMPANY, AND PAYOUT TABLE WERE UPDATED'
																);
															}
															functions
																.CheckForActive(
...
module.exports.help = {
	...
	description:
		'Calculate payout and rank up of a member when they turn in vouchers',
	args: [
		{
			name: 'create',
		...
		},
		{
			...
			parse: (bot, message, args) => {
				if (!args[0]) return null;
				return args[0].toLowerCase() == 'd' || args[0].toLowerCase() == 'deny'
					? 'deny'
					: null;
			},
		},
