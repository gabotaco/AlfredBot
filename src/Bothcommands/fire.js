...
module.exports.run = async (bot, args) => {
	return new Promise((resolve, reject) => {
		...
		if (args.reason.length > 120)
			return resolve(`Please shorten the leave reason to 120 characters`); //limit is 120 characters

		if (args.reason.includes("'")) return resolve("Please no ' characters"); //can't have lil quotes

		bot.con.query(
			`UPDATE members SET company = 'fired', fire_reason = '${
				args.reason
			}', deadline = '${new Date()
				.toISOString()
				.slice(0, 19)
				.replace('T', ' ')}' WHERE ${SearchColumn} = '${FireMember}'`,
			function (err, result, fields) {
				//update their company to fired and add fire reason and set deadline to the fired date
				if (err) {
					console.log(err);
					return reject("Couldn't update member.");
				}
				if (result.affectedRows == 0)
					return resolve('Unable to find that member');
				else {
					resolve('Fired.');
					bot.con.query(
						`SELECT discord_id FROM members WHERE ${SearchColumn} = '${FireMember}'`,
						function (err, result, fields) {
							if (err) {
