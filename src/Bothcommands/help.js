...
module.exports.run = async (bot, args) => {
    return new Promise((resolve, reject) => {
        if (args.guild_id == botconfig.PIGSServer) {
            ...
            var MemberRoles = bot.guilds.cache.get(botconfig.PIGSServer).members.cache.get(args.author_id)
        } else if (args.guild_id == botconfig.RTSServer) {
            var CommandFolder = "./RTScommands/" //folder with rts commands
            var MemberRoles = bot.guilds.cache.get(botconfig.RTSServer).members.cache.get(args.author_id)
        }
        ...
        fs.readdir(CommandFolder, (err, files) => { //reads all files in the commands folder
            ...
            jsfile.forEach((f) => { //for each file in commands
                ...
                if (i < 25 && canUseCommand(MemberRoles, command.help.permission)) { //If i haven't added 25 fields
                    help1.addField(`.${command.help.name} ${command.help.usage}`, `${command.help.description}.`) //adds the command info to the embed
                    i++ //adds 1 to I
                } else if (i < 50 && canUseCommand(MemberRoles, command.help.permission)) { //need 2 because you can't have more than 25 fields in an embed or else it fails
            ...
            fs.readdir("./Bothcommands", (err, files) => { //reads all files in the commands folder
                ...
                jsfile.forEach((f) => { //for each file in commands
                    ...
                    if (command.help.disabled) return;
                    if (i < 25 && canUseCommand(MemberRoles, command.help.permission)) { //Doesn't go over 25
                        help1.addField(`.${command.help.name} ${command.help.usage}`, `${command.help.description}.`) //adds the command info to the embed
                        i++ //adds 1 to I
                    } else if (i < 50 && canUseCommand(MemberRoles, command.help.permission)) { //need 2 because you can't have more than 25 fields in an embed or else it fails
                        help2.addField(`.${command.help.name} ${command.help.usage}`, `${command.help.description}.`)
