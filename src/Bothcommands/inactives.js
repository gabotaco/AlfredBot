			...
			function (err, result, fields) {
				...
				result.forEach(member => {
					const DiscordMember = guild.members.cache.get(
						member.discord_id.toString()
					); //find member in discord
					const D3 = D2 - new Date(member.deadline); //difference between deadline and today
					if (!DiscordMember) {
						errors.push(
							`<@${member.discord_id}> ${member.in_game_name} is no longer in the server...`
						);
						return;
					}
					if (D3 >= 0) {
						//if past deadline
						if (!DiscordMember.roles.cache.has(InactiveRole))
							DiscordMember.roles.add(InactiveRole); //if the member is in discord and doesn't have inactive role then add inactive role
						if (args.notify) {
							...
							DiscordMember.send(
								...
								errors.push(
									`Couldn't dm <@${DiscordMember.id}>. They probably disabled it.`
								)
							); //notify and if can't tell us
						}
						...
						if (FieldsAdded < 25) {
							...
							last.addField(
								`${member.in_game_name} (${member.discord_id})`,
								`Last turnin: ${member.last_turnin}`,
								false
							); //embed 1

							discordIDS.push(member.discord_id); //push discord id's
						} else if (FieldsAdded >= 25 && FieldsAdded < 50) {
						...
						} else if (FieldsAdded >= 50 && FieldsAdded < 75) {
				...
				if (!args.notify) {
					//If no args
					if (last.fields[0]) {
						//If there are fields in embed 1
						response.push(last); //send 1
					} else {
						//No fields in embed 1
...
module.exports.help = {
	...
	usage: '[notify]',
	description:
		"Sends a list of members who haven't turned in over the past 2 weeks",
	args: [
		{
			type: 5,
			name: 'notify',
