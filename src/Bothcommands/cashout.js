...
module.exports.run = async (bot, args) => {
	return new Promise((resolve, reject) => {
		if (args.author_id == '404650985529540618') {
			...
			bot.con.query(
				`SELECT ${CompanyName}_cashout, ${CompanyName}_cashout_worth FROM managers ma, members me WHERE me.${SearchColumn}='${args.author_id}' AND ma.member_id = me.id`,
				function (err, result, fields) {
						...
						.setTitle(
							`Cashout for ${
								SearchColumn == 'discord_id'
									? bot.guilds.cache
											.get(args.guild_id)
											.members.cache.get(args.author_id).displayName
									: args.author_id
							}`
						)
						.addField(
