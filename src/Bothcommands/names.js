    ...
    bot.con.query(
      ...
      function (err, result, fields) {
        ...
        setTimeout(() => {
          //wait 5000 ms
          if (!SentMessage) {
            //if not sent message after 5 seconds
            return resolve("Server isn't online.");
          }
        }, 5000);

        request(
          `http://${Server}/widget/players.json`,
          { json: true },
          function (error, response, body) {
            ...
            if (!error && body) {
              ...
              body.players.forEach((player) => {
                ...
                if (CompanyMembers.includes(player[2].toString())) {
                  //if the player is in company
              ...
              if (playerNames.length < 1)
                return resolve("No players on that server."); //if less than 1 player then no players
