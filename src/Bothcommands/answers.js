...
module.exports.run = async (bot, args) => {
  return new Promise((resolve, reject) => {
    let AnswersEmbed = new Discord.MessageEmbed().setColor("RANDOM");

    const SearchColumn = functions.GetSearchColumn(args.id || args.member);

    bot.con.query(
      `SELECT in_game_name, why, anything, play_per_week, id, status FROM applications WHERE ${SearchColumn} = '${
        args.id || args.member
      }'`,
      function (err, result, fields) {
        ...
        result.forEach((applicant) => {
          ...
          AnswersEmbed.addField(
            "This sounds serious but it's totally not! Why should we choose you?",
            applicant.why
          );
          if (applicant.anything)
            AnswersEmbed.addField(
              "Say anything! (Hobbies, interests, field of work, whatever makes you, you!)",
              applicant.anything
            );
          AnswersEmbed.addField(
            "How much do you play per week right now?",
            applicant.play_per_week
          );
