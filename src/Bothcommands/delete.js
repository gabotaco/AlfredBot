...
module.exports.run = async (bot, args) => {
	return new Promise((resolve, reject) => {
		...
		bot.con.query(
			`SELECT id FROM members WHERE ${SearchColumn} = '${user}'`,
			function (err, results, fields) {
				...
				results.forEach(result => {
					bot.con.query(
						`DELETE FROM members WHERE id = '${result.id}'`,
						function (err, res, fields) {
							...
							bot.con.query(
								`DELETE FROM pigs WHERE member_id = '${result.id}'`,
								function (err, res, fields) {
									if (err) {
										console.log(err);
									...
									bot.con.query(
										`DELETE FROM rts WHERE member_id = '${result.id}'`,
										function (err, res, fields) {
											if (err) {
												console.log(err);
