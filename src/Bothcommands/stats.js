	...
	return new Promise((resolve, reject) => {
		...
		if (args.sub_command_group.toLowerCase() == 'total') {
			if (args.sub_command.toLowerCase() == 'days') {
				...
				bot.con.query(
					`SELECT * FROM payout WHERE createdAt > '${Threshold.toISOString()}'`,
					function (err, result, fields) {
						...
						const statEmbed = new Discord.MessageEmbed()
							.setTitle(
								`Total vouchers between now and ${parseInt(args.num)} days ago`
							)
							.setColor('RANDOM')
							.addField(
								'Total Vouchers',
								functions.numberWithCommas(TotalVouchers),
								true
							)
							.addField(
								'Cashout value',
								'$' +
									functions.numberWithCommas(
										Math.abs(
											TotalVouchers * 10000 - TotalVouchers * 10000 - TotalValue
										)
									),
								true
							)
							.addField(
								...
								'$' +
									functions.numberWithCommas(
										TotalVouchers * 10000 - TotalValue
									),
								true
							);
				...
				bot.con.query(`SELECT * FROM managers`, function (err, result, fields) {
					//get all their info into one array
					if (err) {
						console.log(err);
					...
					const statEmbed = new Discord.MessageEmbed()
						...
						.addField(
							'Total value',
							'$' + functions.numberWithCommas(TotalValue),
							true
						);
					return resolve(statEmbed);
				});
		...
		if (args.sub_command.toLowerCase() == 'days') {
			...
			Threshold.setDate(Threshold.getDate() - parseInt(args.num));
			bot.con.query(
				`SELECT * FROM payout WHERE createdAt > '${Threshold.toISOString()}' AND company = '${company}'`,
				function (err, result, fields) {
					//get all their info into one array
					if (err) {
						console.log(err);
					...
					const statEmbed = new Discord.MessageEmbed()
						.setTitle(
							`Total vouchers between now and ${parseInt(
								args.num
							)} days ago for ${company.toUpperCase()}`
						)
						.setColor('RANDOM')
						.addField(
							'Total Vouchers',
						...
						.addField(
							'Cashout Profit',
							'$' +
								functions.numberWithCommas(TotalVouchers * 10000 - TotalValue),
							true
						);
					return resolve(statEmbed);
				}
			...
			bot.con.query(`SELECT * FROM managers`, function (err, result, fields) {
				...
				const statEmbed = new Discord.MessageEmbed()
					.setTitle(
						`Total unpaid vouchers with ${company.toUpperCase()}'s managers`
					)
					.setColor('RANDOM')
					.addField(
						'Total Vouchers',
			...
			if (NumOfPlayers > 25)
				return resolve('Too many players. The limit is 25.');
			const Threshold = new Date();
			Threshold.setDate(Threshold.getDate() - NumOfDays);

			bot.con.query(
				`SELECT * FROM members me, payout pa WHERE pa.company = '${company}' AND me.id = pa.member_id AND pa.createdAt > '${Threshold.toISOString()}' ORDER BY pa.member_id DESC`,
				function (err, result, fields) {
					//get all their info into one array
					if (err) {
						console.log(err);
					...
					for (let i = 0; i < NumOfPlayers && i < TopPlayers.length; i++) {
						TopEmbed.addField(
							TopPlayers[i].in_game_name + ` (${i + 1})`,
							functions.numberWithCommas(TopPlayers[i].amount),
							true
						);
					}
			...
			options: [
				{
					name: 'days',
					description:
						'Get total vouchers and worth over the past number of days',
					type: 1,
					options: [
						{
							name: 'num',
		...
		},
		{
			...
			options: [
				...
				},
				{
					name: 'unpaid',
					description:
						'Get the total amount and worth of unpaid vouchers in rts',
					type: 1,
					options: [],
					parse: (bot, message, args) => {
						if (!args[1]) return null;
		...
		},
		{
			...
			options: [
				...
				},
				{
					name: 'unpaid',
					description:
						'Get the total amount and worth of unpaid vouchers in pigs',
					type: 1,
					options: [],
					parse: (bot, message, args) => {
						if (!args[1]) return null;
