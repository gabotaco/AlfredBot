...
app.get("/roles/inactive", function (req, res) {
    ...
    bot.guilds.cache.get(req.query.server).members.cache.get(req.query.member).roles.remove(req.query.server == botconfig.RTSServer ? botconfig.RTSRoles.InactiveRole : botconfig.PIGSRoles.InactiveRole);
...
bot.on("ready", async () => { //When the bot logs in
    ...
    fs.readdir("./src/Bothcommands/", (err, files) => { //Gets all files in the Bothcommands folder
    ...
    fs.readdir("./src/RTScommands/", (err, files) => { //Gets all files in the RTScommands folder
