...
function optionsAreSame(options1, options2) {
	...
	for (let i = 0; i < options1.length; i++) {
		if (
			options1[i].type == options2[i].type &&
			options1[i].name == options2[i].name &&
			options1[i].description == options2[i].description
		) {
			if (!optionsAreSame(options1[i].options, options2[i].options))
				return false;
		} else {
			return false;
		}
...
slash.init = async client => {
		...
		return new Promise(async (resolve, reject) => {
			...
			if (typeof response === 'object') {
				if (response.messageOptions) {
					data = await slash.createAPIMessage(
						interaction,
						response.message,
						response.messageOptions
					);
				} else {
	...
	slash.edit = async (interaction, response, command) => {
		return new Promise(async (resolve, reject) => {
			...
			request.patch(
				`https://discord.com/api/v8/webhooks/${slash.app.id}/${interaction.token}/messages/@original`,
				{
					body: data,
					headers: {
						Authorization: `Bot ${process.env.BOT_TOKEN}`,
	...
	slash.follow = async (interaction, response, command) => {
		return new Promise(async (resolve, reject) => {
			...
			request.post(
				`https://discord.com/api/v8/webhooks/${slash.app.id}/${interaction.token}`,
				{
					body: data,
					headers: {
						Authorization: `Bot ${process.env.BOT_TOKEN}`,
		...
		if (command) {
			...
			command
				...
				.catch(err => {
					slash.reply(
						interaction,
						`There was an error. Gabo says ${err}`,
						command
					);
				});
		} else {
