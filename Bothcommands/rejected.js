...
module.exports.run = async (bot, args) => {
  return new Promise((resolve, reject) => {
    ...
    bot.con.query(`SELECT * FROM applications WHERE ${SearchColumn} = '${ID}'`, function (err, result) {
      if (err) {
        console.log(err)
        reject("Unable to get applicants from DB.")
      } else {
        if (result.length > 0) {
          functions.UpdateApplicantStatus(bot.con, ID, `Rejected`, LeaveReason) //update status
          const user = bot.guilds.cache.get(args.guild_id).members.cache.get(result[0].discord_id) //get server member
