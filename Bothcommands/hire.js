...
module.exports.run = async (bot, message, args) => {
  ...
  if (args[0] && !args[1]) { //if they only have 1 arg
    bot.con.query(`SELECT discord_id, in_game_name, in_game_id FROM applications WHERE in_game_id = ${args[0]}`, function (err, result) {
      if (err) {
        console.log(err)
        message.channel.send("There was an error")
      } else {
        if (result.length > 0) {
          Hire(result[0].discord_id, result[0].in_game_name, result[0].in_game_id) //Hire em
        } else {
  ...
  async function Hire(DiscordID, InGameName, InGameID) {
    ...
    functions.UpdateApplicantStatus(bot.con, message.channel, InGameID, "Hired")
