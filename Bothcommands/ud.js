...
module.exports.run = async (bot, args) => {
  return new Promise(async (resolve, reject) => {
    const res = await urban(args.word).catch(e => { //search urban dictionary and if there is an error then that means there were no responses
      resolve("Did you make that up or something because its not even on urban dictionary");
      return;
    });
