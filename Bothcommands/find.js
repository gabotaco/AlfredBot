...
module.exports.run = async (bot, args) => {
  return new Promise((resolve, reject) => {
    if (args.sub_command == 'manager') {
      const Managers = {};
      bot.con.query(`SELECT me.in_game_id, me.company FROM members me, managers ma WHERE me.id = ma.member_id AND ma.active = '1'`, function (err, result, fields) {
        if (err) {
          console.log(err)
      ...
      bot.con.query(`SELECT (in_game_id) FROM members WHERE ${SearchColumn} = '${args.id || args.member}'`, function (err, result, fields) { //get all members
        if (err) {
          console.log(err)
          ...
          function tryGetInGame(index) {
            ...
            request(`https://${botconfig.ActiveServers[index].url}/status/snowflake2user/${args.member || args.id}`, {
              ...
              if (response.user_id) {
                ...
                return resolve("That user does not have their discord linked.")
              }
            })
          }
