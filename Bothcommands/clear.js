module.exports.run = async (bot, message, args) => {
    ...
    try {
        message.channel.bulkDelete(args[0]).then(() => { //deletes the number of messages specified
            message.channel.send(`Cleared ${args[0]} cancer cells`).then(msg => msg.delete(1000)) //then sends a message saying that it deleted the cancer cells. Then deletes the message after 1000 milliseconds
        });
