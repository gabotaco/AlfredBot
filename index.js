...
bot.on("presenceUpdate", (oldPresence, newPresence) => { //When a guild member's presence changes (online/offline or games)
    ...
    if (oldPresence.member.hasPermission("KICK_MEMBERS") && newPresence.guild.id == botconfig.PIGSServer && !newPresence.user.bot) { //if its a pigs manager and the update is triggered in the pigs server
        ...
        else if (newPresence.status == "online" && newPresence.member.roles.cache.has(botconfig.PIGSUnavailableRole) && (optOutUnavailable.includes(newPresence.member.id))) return newPresence.member.roles.remove(botconfig.PIGSUnavailableRole) //If they are now online and have the unavailable role and are alt tabs or solid 2 hours it will auto make em available
        ...
        else if (newPresence.status == "online" && newPresence.member.roles.cache.has(botconfig.RTSUnavailableRole) && (optOutUnavailable.includes(newPresence.member.id))) return newPresence.member.roles.remove(botconfig.RTSUnavailableRole) //If they are now online and have the unavailable role and are alt tabs or solid 2 hours it will auto make em available
